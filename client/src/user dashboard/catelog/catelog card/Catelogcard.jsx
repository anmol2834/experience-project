...
function Catelogcard({ title, state, city, price, img, stock, mrp, ratings, productId, isLiked }) {
  const { token } = useAuth();
  const [like, setLike] = useState(isLiked); // Use prop as initial state
  const [wishlistLoading, setWishlistLoading] = useState(false); // For toggling actions
